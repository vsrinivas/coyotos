...
compute_indirect_bytes(SymVec& symVec)
{
  ...
  for(size_t i = 0; i < symVec.size(); i++) {
    ...
    sz = round_up(sz, sym->alignof(*targetArch));
    sz += sym->indirectSize(*targetArch);
  }
			...
			unsigned align)
{
  ...
  case sc_arrayType:
    {
      /* No indirect size */
      unsigned elemAlign = s->alignof(*targetArch);
      unsigned elemSize = s->directSize(*targetArch);
			  ...
			  std::ostream& out, int indent, SymClass sc, unsigned align)
{
  ...
  case sc_bufType:
    {
      unsigned elemAlign = s->type->alignof(*targetArch);
      unsigned elemSize = s->type->directSize(*targetArch);
      elemSize = round_up(elemSize, elemAlign);
  ...
  if (symVec.size() == 1 &&
    ...
    for(size_t i = 0; i < symVec.size(); i++) {
      ...
      align = emit_symbol_align("sndLen", out, indent+2,
				argBaseType->alignof(*targetArch), align);
	...
	if (argBaseType->IsVarSequenceType()) {
	  ...
	  indirAlign = emit_symbol_align("sndIndir", out, indent+2,
				    argBaseType->alignof(*targetArch), indirAlign);
