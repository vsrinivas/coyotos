...
emit_op_dispatcher(GCPtr<Symbol> s, std::ostream& out)
{
  ...
  if (rcvRegs.size() || rcvString.size()) {
	...
	rcvOffset = round_up(rcvOffset, argBaseType->alignof(*targetArch));
	...
	rcvOffset += argBaseType->directSize(*targetArch);
      }
    }
  ...
  if (sndString.size() || sndRegs.size()) {
	...
	sndOffset += argBaseType->directSize(*targetArch);
      }
    }
	...
	sndOffset = round_up(sndOffset, argBaseType->alignof(*targetArch));
  ...
  if (sndRegs.size() || sndString.size()) {
	...
	unsigned elemAlign = argBaseType->alignof(*targetArch);
