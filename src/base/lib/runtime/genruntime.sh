...
while read def name value; do
	...
	coyotos_RunTime_TOOL_*)
        ...
        ;;
	coyotos_RunTime_REGNO_*)
	    reg="${name#coyotos_RunTime_REGNO_}"
	    echo "#define CRN_${reg} 	${value}"
        ;;
...
#define COYOTOS_NUM_APPREGS (CRN_LASTAPP - CRN_FIRSTAPP + 1)
...
#define CR_APP(x)	REG_CAPLOC((x) + CRN_FIRSTAPP)
#define CR_REPLY(x)	REG_CAPLOC((x) + CRN_REPLY0) /* 0 <= x <= 3 */
#define CR_ARG(x)	REG_CAPLOC((x) + CRN_ARG0)   /* 0 <= x <= 2 */
