...
#include <coyotos.stream.Null.h>

/* Utility quasi-syntax */
#define unless(x) if (!(x))

#define CR_LOG   		coyotos_stream_Null_APP_Log
#define CR_MYENTRY   		coyotos_stream_Null_APP_MyEntry

#define TOOL_LOG 		coyotos_stream_Null_TOOL_Log
...
HANDLE_coyotos_IoStream_getReadChannel(
  ...
  struct IDL_SERVER_Environment *_env)
{
  /* Since Null never blocks, there is no need to actually maintain
