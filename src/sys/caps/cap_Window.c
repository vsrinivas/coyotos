...
#include <idl/coyotos/Window.h>

extern void cap_Memory(InvParam_t* iParam);

void
cap_Window(InvParam_t *iParam)
{
  ...
  case OC_coyotos_Cap_getType:	/* Must override. */
    {
      ...
      sched_commit_point();
      InvTypeMessage(iParam, IKT_coyotos_Window);
      return;
    }

  case OC_coyotos_Window_getOffset:
    {
      INV_REQUIRE_ARGS(iParam, 0);

      sched_commit_point();
      put_oparam64(iParam, iParam->iCap.cap->u2.offset);
