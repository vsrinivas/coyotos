...
C_DEP=$(GCC) $(GCCFLAGS) $(GCCWARN) $(COYOTOS_WARN_ERROR) -M -MT $@ -MF $(BUILDDIR)/.$(patsubst %.o,%.m,$(notdir $@)) $<
CXX_DEP=@$(GPLUS) $(GPLUSFLAGS) $(GPLUSWARN) $(COYOTOS_WARN_ERROR) -M -MT $@ -MF $(BUILDDIR)/.$(patsubst %.o,%.m,$(notdir $@)) $<
ASM_DEP=@$(GCC) $(ASMFLAGS) -M -MT $@ -MF $(BUILDDIR)/.$(patsubst %.o,%.m,$(notdir $@)) $<
#MOPS_DEP=$(GCC) $(GCCFLAGS) $(MOPSWARN) $(COYOTOS_WARN_ERROR) -S $< -o $(BUILDDIR)/.$(patsubst %.o,%.m,$(notdir $@))

C_BUILD=$(GCC) $(GCCFLAGS) $(GCCWARN) $(COYOTOS_WARN_ERROR) -c $< -o $@
CXX_BUILD=$(GPLUS) $(GPLUSFLAGS) $(GPLUSWARN) $(COYOTOS_WARN_ERROR) -c $< -o $@
ASM_BUILD=$(GCC) $(ASMFLAGS) -c $< -o $@
MOPS_BUILD=$(GCC) -B$(MOPS)/rc/ $(GCCFLAGS) $(MOPSWARN) $(COYOTOS_WARN_ERROR) -S $< -o $@
